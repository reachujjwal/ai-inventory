...
export default function AddOrder() {
    ...
    useEffect(() => {
        if (!loading && !isAuthenticated) {
            router.push('/login');
        } else if (!loading && isAuthenticated && !hasPermission(user?.permissions, PERMISSIONS.MODULE_ORDERS, 'can_add')) {
            router.push('/');
    ...
    const estimatedTotal = selectedProduct ? (selectedProduct.price * form.quantity).toFixed(2) : '0.00';

    if (loading || !isAuthenticated || !hasPermission(user?.permissions, PERMISSIONS.MODULE_ORDERS, 'can_add')) {
        return (
            ...
            <div className="max-w-2xl mx-auto">
                <div className="bg-surface border border-border rounded-xl p-6 shadow-sm">
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div className="bg-background/50 p-4 rounded-lg flex justify-between items-center border border-border">

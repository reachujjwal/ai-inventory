...
export default function EditProduct() {
    ...
    useEffect(() => {
        if (!loading && !isAuthenticated) {
            router.push('/login');
        } else if (!loading && isAuthenticated && !hasPermission(user?.permissions, PERMISSIONS.MODULE_PRODUCTS, 'can_update')) {
            router.push('/');
        } else if (isAuthenticated && hasPermission(user?.permissions, PERMISSIONS.MODULE_PRODUCTS, 'can_update') && id) {
            fetchCategories();
            fetchProduct();
        }
    ...
    if (loading || !isAuthenticated || !hasPermission(user?.permissions, PERMISSIONS.MODULE_PRODUCTS, 'can_update')) {
        return (
                ...
                <div className="bg-surface border border-border rounded-xl p-6 shadow-sm">
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div>
                            ...
                            {imagePreview && (
                                <div className="mt-3">
                                    <p className="text-xs text-text-secondary mb-2">New Image Preview:</p>
